...
|---|---|---|
| **2026-07-30** | **UI & Branding** | Added the new OC gradient logo icon to the sidebar header (`SidebarHistory.tsx`) and removed the text. Processed logo assets via Pillow to make the background 100% transparent so that the logo blends perfectly in both dark and light themes without white borders/squares. Updated cache-busting queries to `v=7` for the assets in `index.html`. Removed all action/banner/submit/header icons from Sign In, Create Account forms and Chat section headers. Cleaned up legacy `VITE_API_BASE_URL` secret references from `vercel.json` and dashboard settings to enable forced clean production builds. |
