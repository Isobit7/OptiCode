...
|---|---|---|
| **2026-07-30** | **UI & Branding** | Added the new OC gradient logo icon next to the OptiCode branding in the sidebar header (`SidebarHistory.tsx`). Updated all website favicon assets (`favicon.ico`, `favicon-16x16.png`, `favicon-32x32.png`, `apple-touch-icon.png`, `logo.png`, `logo.webp`) using Pillow processing to use the new OC gradient logo. Removed all action/banner/submit/header icons from Sign In, Create Account forms and Chat section headers. Cleaned up legacy `VITE_API_BASE_URL` secret references from `vercel.json` and dashboard settings to enable forced clean production builds. |
