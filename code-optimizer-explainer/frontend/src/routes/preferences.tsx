...
import type { ExplainDepth } from "@/components/optimizer/modals/PreferencesDropdown";
...
export const Route = createFileRoute("/preferences")({
  head: () => ({
    meta: [
      { title: "Setup Your Workspace — OptiCode" },
      {
        name: "description",
        content: "Personalize your OptiCode workspace with your preferences.",
      },
    ],
  }),
...
function PreferencesRoute() {
  ...
  const handleFinish = () => {
    ...
    try {
      localStorage.setItem("opticode_user_onboarding_prefs", JSON.stringify(prefs));
    } catch (err) {
      void err;
    }
  ...
  return (
    <div className="min-h-screen bg-[#0e0e14] flex items-center justify-center p-4">
      ...
      <div className="w-full max-w-2xl bg-zinc-950 text-zinc-100 border border-zinc-800 shadow-2xl rounded-2xl overflow-hidden">
        {/* Header Banner & Stepper Progress */}
        <div className="bg-gradient-to-r from-orange-500/10 via-amber-500/10 to-transparent p-6 pb-4 border-b border-zinc-800/80">
          <div className="flex items-center gap-2 text-orange-400 text-xs font-bold uppercase tracking-wider mb-2">
            <Sparkles className="h-4 w-4 shrink-0" />
            <span>Welcome to OptiCode Setup</span>
